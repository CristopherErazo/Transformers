...
from attention_nn.dataset import get_random_dataloader
...
def main():
    ...
    parser.add_argument('--Pdat', type=str, default='tiny', help='Type of token distribution: uni (uniform) or tiny (from TinyStories).')
    ...
    variable_params = { key : config[key] for key in ['sigma','type_enc','amp','unmb','Pdat']}
    ...
    TinyStories_statistics = load_data('token_counts','data_statistics',params=params_stats,base_dir='./data')
    config['vocab_size'] = TinyStories_statistics['sorted_token_counts'].shape[1]
    for key in TinyStories_statistics.keys():
            print(f'{key} : {TinyStories_statistics[key].shape}')

    if config['Pdat'] == 'uni':
        P_tokens = np.ones(config['vocab_size']) / config['vocab_size']
    elif config['Pdat'] == 'tiny':
        P_tokens = TinyStories_statistics['sorted_token_counts'].sum(axis=0)  # (vocab_size,)
        P_tokens = P_tokens / P_tokens.sum()
    else:
        raise ValueError(f'Unknown P type: {config["P"]}')

    data_config = {k:config[k] for k in ['dataset_size','vocab_size','train_fraction','batch_size','L']}
    data_config['P_tokens'] = P_tokens

    # Prepare data loaders and tokenizer
    print('Preparing data loaders...')
    train_dataloader, val_dataloader = get_random_dataloader(data_config)
    ...
    if config['Pdat'] == 'uni':
        P_mu = np.ones((config['L']-1,config['vocab_size'])) / config['vocab_size']
        P_tot = np.ones((config['vocab_size'],)) / config['vocab_size']
        H_mu = np.log(config['vocab_size']) * np.ones((config['L']-1,))
        H_tot = np.log(config['vocab_size'])
    elif config['Pdat'] == 'tiny':
        P_tot = P_tokens
        # P_mu = P tokens replicated L-1 times
        P_mu = np.tile(P_tokens, (config['L']-1,1))  # (L-1,V)
        H_tot = -np.nansum(P_tot * np.log(P_tot + 1e-12)) 
        H_mu = H_tot * np.ones((config['L']-1,))

    data_stats = (P_mu, P_tot, H_mu, H_tot)
    sorted_rank = np.arange(config['vocab_size']) # (V,)
    ...
    sequence_fractions = [0.2, 0.4, 0.6, 0.8, 0.99]
    ...
    for epoch in range(config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            if measure_matrices:
                ...
                tokens_attended =[ [ [f'{element}' for element in seqfrac_element] 
    ...
    save_data(summary,'summary_log_random_dataset',experiment_name='new_evolution_scalar', params=params)
    save_data(data_matrices,'summary_log_random_dataset',experiment_name='new_evolution_matrices', params=params)
